...
def main(args):
    ...
    class svd_dataset(Dataset):
        def __init__(self,data_path_list,classes,data_num,training,mfcc_params,transform=None,normalize=None):
        ...
        def __getitem__(self, idx):
            ...
            MFCC = T.MFCC(
                            ...
                            melkwargs={
                                'n_fft': self.mfcc_params["win_length"]+self.mfcc_params["n_fft"],
    ...
    def model_initialize(n_mfcc):
        model = MfccDense( in_dim=n_mfcc-1 ).cuda()
        return model
    ...
    def all_train(args):
        ...
        data_ind = 1
        check_path ='../checkpoint/mean-mfcc_sweep_'+str(args.seed)+'_organics_speaker.pt'
        print(check_path)
        ...
        model=model_initialize(wandb.config.n_mfcc)
        criterion = nn.CrossEntropyLoss()
